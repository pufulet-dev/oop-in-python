...
class Barista:
    def makeCoffee(self, orders):
        ...
        for order in orders:
            match order['type']:
                case 1:
                    americano = Americano(order['intensity'], order['ml_of_water'], order['name'])
                    americano.makeAmericano()
                    coffee_list.append(americano)
                case 2:
                    cappuccino = Cappuccino(order['intensity'], order['ml_of_milk'], order['name'])
                    cappuccino.makeCappuccino()
                    coffee_list.append(cappuccino)
                case 3:
                    pumpkin_spice_latte = PumpkinSpiceLatte(order['intensity'], order['ml_of_milk'], order['mg_of_pumpkin_spice'], order['name'])
                    pumpkin_spice_latte.makePumpkinSpiceLatte()
                    coffee_list.append(pumpkin_spice_latte)
                case 4:
                    syrup_cappuccino = SyrupCappuccino(order['intensity'], order['ml_of_milk'], order['syrup'], order['name'])
